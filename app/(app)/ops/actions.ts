...
export async function generateHandoff(input: { rfqId: string }) {
  ...
  // Trade-status drift (e.g. handoff already generated by another tab) is
  // caught by the conditional UPDATE inside the tx — see the
  // captured -> sent guard on step 2 of apply(tx).

  const stpTrades: StpTradeInput[] = tradeRows.map((t) => ({
    ref: t.ref,
    ...
    async (tx) => {
      ...
      const [inserted] = await tx
        ...
        .values({
          ...
          channel: STP_CHANNEL,
          payloadLabel: payloadLabel(stpTrades.length),
          payload,
          status: 'sent',
        })
...
export async function advanceHandoff(input: { handoffId: string; to: 'matched' | 'affirmed' }) {
  ...
  // tradeIds is jsonb NOT NULL (db/schema.ts:252); if it ever comes back
  // null we want a loud failure, not a silent zero-leg advance.
  if (h.tradeIds == null) throw new Error(`Handoff ${handoffId} has null tradeIds.`);
      ...
      const [updated] = await tx
        ...
        .where(
          and(
            ...
            // still true under serializable isolation). Drizzle typed
            // subquery — the equivalent SQL is generated by the builder.
            inArray(
